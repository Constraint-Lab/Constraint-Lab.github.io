...
Code reviews assumed there was a human on the other side we could jump on a call with and explain how it works. We assumed that the twenty commits going out in the next release were backed by ~20 people how understood each commit. It was safe for our all of our rules to assume this, so every rule potentially needs to be changed. This assumption was load-bearing (a human person typed that, relax, it's just a cheap joke).
